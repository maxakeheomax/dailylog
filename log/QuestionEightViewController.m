...
-(IBAction)yes:(id)sender{
    ...
    appdelegate.questionEightData  = @"Yes";
...
-(IBAction)no:(id)sender{
    AppDelegate *appdelegate = (AppDelegate *)[[UIApplication sharedApplication]delegate];
    appdelegate.questionEightData  = @"No";
    UIToolbar *toolsR = [[UIToolbar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 103.0f, 44.01f)];
    toolsR.clearsContextBeforeDrawing = NO;
    toolsR.clipsToBounds = NO;
    toolsR.tintColor = [UIColor colorWithWhite:0.305f alpha:0.0f];
...
-(void)forward{
    QuestionNineViewController *ninth = [[QuestionNineViewController alloc]init];
    [self.navigationController pushViewController:ninth animated:YES];
}
- (void)viewDidUnload
{
    [super viewDidUnload];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"Question8NavBar.png"] forBarMetrics:UIBarMetricsDefault];
}
