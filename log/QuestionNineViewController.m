...
#import "QuestionNineViewController.h"
#import "QuestionTenViewController.h"
#import "AppDelegate.h"

@interface QuestionNineViewController ()

@end

@implementation QuestionNineViewController
...
-(IBAction)yes:(id)sender{
    ...
    appdelegate.questionNineData  = @"Yes";
...
-(IBAction)no:(id)sender{
    AppDelegate *appdelegate = (AppDelegate *)[[UIApplication sharedApplication]delegate];
    appdelegate.questionNineData  = @"No";
    UIToolbar *toolsR = [[UIToolbar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 103.0f, 44.01f)];
    toolsR.clearsContextBeforeDrawing = NO;
    toolsR.clipsToBounds = NO;
    toolsR.tintColor = [UIColor colorWithWhite:0.305f alpha:0.0f];
...
-(void)forward{
    QuestionTenViewController *tenth = [[QuestionTenViewController alloc]init];
    [self.navigationController pushViewController:tenth animated:YES];
}
- (void)viewDidUnload
{
    [super viewDidUnload];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"Question9NavBar.png"] forBarMetrics:UIBarMetricsDefault];
}
